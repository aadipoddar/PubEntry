...
public class TransactionData
{
	...
	public static async Task<List<TransactionModel>> GetTransactionsByDateRangeAndLocation(string fromDateTime, string toDateTime, int locationId) =>
		await SqlDataAccess.LoadDataSQL<TransactionModel>($"SELECT * FROM TransactionTable WHERE DateTime >= '{fromDateTime}' AND DateTime <= '{toDateTime}' AND LocationId = {locationId}", Secrets.DatabaseName) as List<TransactionModel>;
}
